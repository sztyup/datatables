...
use Sztyup\Datatable\AddIfTrait;
...
abstract class AbstractColumn implements ColumnInterface
{
    ...
    public function isToManyAssociation()
    {
        if (true === $this->isAssociation() && null !== $this->typeOfAssociation) {
            if (in_array(ClassMetadataInfo::ONE_TO_MANY, $this->typeOfAssociation) ||
                in_array(ClassMetadataInfo::MANY_TO_MANY, $this->typeOfAssociation)
            ) {
                return true;
            } else {
                return false;
    ...
    {
        return 'datatables::column.column';
    }
    ...
    public function renderCellContent(array &$row)
    {
        if (is_callable($this->getCalc())) {
            data_set($row, $this->dql, call_user_func_array($this->getCalc(), [&$row]));
        }

...
use Sztyup\Datatable\Filter\AbstractFilter;
use Sztyup\Datatable\Filter\FilterInterface;
...
class DatatableQueryBuilder
{
    ...
    private function setLimit(QueryBuilder $qb)
    {
        if (true === $this->features->getPaging() || null === $this->features->getPaging()) {
            if (isset($this->requestParams['start']) &&
                DatatableQueryBuilder::DISABLE_PAGINATION != $this->requestParams['length']
            ) {
    ...
    /**
     * Set wheter or not to cache result of records retrieval query and if so, for how long and under which ID.
     * Method is consistent with {@see \Doctrine\ORM\AbstractQuery::useResultCache} method.
     *
     * @param bool        $bool          flag defining whether use caching or not
     * @param int|null    $lifetime      lifetime of cache in seconds
     * @param string|null $resultCacheId identifier for result cache if left empty ID will be generated by Doctrine
     *
     * @return $this
     */
    ...
    {
        $searchColumn =
            null !== $this->accessor->getValue($column, 'dql') &&
            true === $this->accessor->getValue($column, 'searchable');
